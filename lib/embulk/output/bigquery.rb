    ...
    class Bigquery < OutputPlugin
      ...
      def self.configure(config, schema, processor_count)
        task = {
          ...
          'compression'                    => config.param('compression',                    :string,  :default => 'GZIP'),
        ...
        unique_name = "%dx%08x%08x" % [Process.pid, now.tv_sec, now.tv_nsec]
      ...
      def self.transaction(config, schema, processor_count, &control)
        ...
        begin
          ...
          if task['skip_file_generation']
            paths = Dir.glob("#{task['path_prefix']}*#{task['file_ext']}")
